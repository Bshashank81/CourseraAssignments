...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for(int k=0;k<tokens.size();k++)
		{
			...
			String currWord = tokens.get(k);
			
			char lastCharOfWord = currWord.charAt(currWord.length()-1);
			...
			if(isWord(currWord))
			{
		        ...
		        for(int i=0;i<currWord.length();i++)
				{
					...
					if(currWord.endsWith("e")&&(i==currWord.length()-1))
					    break;
				...
				if(count==0&&currWord.endsWith("e"))
				    syllableCount=syllableCount+1;
			}
			if(k==tokens.size()-1&&(Character.isLetter(lastCharOfWord)))
			{
				sentencesCount++;
			}
		}
